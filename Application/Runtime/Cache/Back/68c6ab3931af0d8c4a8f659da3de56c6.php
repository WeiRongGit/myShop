...
<div style="font-size: 13px;margin: 10px 5px">
    <form action="/index.php/Back/Goods/update" method="post" enctype="multipart/form-data">
        <input type="hidden" name='goods_id' value="<?php echo ($good["goods_id"]); ?>"/>
        <table border="1" width="100%" class="table_a" id="general-tab-tb">
            <tr>
                <td>商品名称</td>
                <td><input type="text" name="goods_name" value="<?php echo ($good["goods_name"]); ?>"/></td>
            </tr>
            <tr>
                <td>商品价格</td>
                <td><input type="text" name="goods_price" value="<?php echo ($good["goods_price"]); ?>"/></td>
            </tr>
            <tr>
                <td>商品logo图片</td>
                <td>
        ...
        >
            <tr>
                <td>商品详细描述</td>
                <td>
                    <textarea name="goods_introduce" id='goods_introduce' style="width:730px;height:320px;"><?php echo ($good["goods_introduce"]); ?></textarea>
                </td>
            </tr>
            <script type='text/javascript'>
        ...
        >
            <tr>
                <td>商品重量</td>
                <td><input type="text" name="goods_weight" value="<?php echo ($good["goods_weight"]); ?>"/></td>
            </tr>
        </table>
        ...
        style="display:none;">
            ...
            <?php if(!empty($picsinfo)): ?><style type='text/css'>li {
                ...
                <tr>
                    <td colspan='100'>
                        <ul>
                            <?php if(is_array($picsinfo)): foreach($picsinfo as $key=>$v): ?><li id="pics_<?php echo ($v["id"]); ?>"><img src='<?php echo (deletePoint($v["pics_big"])); ?>' alt='' width='100'
                                                           height='100'/>
                                    <span style='cursor:pointer;'
                                          onclick='if(confirm("确实要删除该相册图片么？")){del_pics(<?php echo ($v["id"]); ?>)}'>[-]</span>
